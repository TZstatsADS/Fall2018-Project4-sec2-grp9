...
final.table <- foreach (txti = 1:100) %dopar% {
  #for(txti in 1:5){
  ...
  load('/Users/brett/Documents/CU/5243 ADS/Project 4/Fall2018-Project4-sec2--sec2proj4_grp9/lib/dict.RData')
  ...
  old_intersect_vec <- vintersect(tolower(ground_truth_vec), tolower(tesseract_vec))
  new_intersect_vec <- vintersect(tolower(ground_truth_vec), tolower(tesseract_vec1))
  
  ocr.error <- length(tesseract_vec) - length(old_intersect_vec)
  total.dect <- length(false_word)
  afterc.error <- length(tesseract_vec) - length(new_intersect_vec)
  before.precision <- length(old_intersect_vec)/length(tesseract_vec)
  before.recall <- length(old_intersect_vec)/length(ground_truth_vec)
  after.recall <- length(new_intersect_vec)/length(ground_truth_vec)
  after.precision <- length(new_intersect_vec)/length(tesseract_vec1)
  print(c(ocr.error, total.dect, afterc.error, before.recall, before.precision, after.recall, after.precision))
}

final.table <- ldply(final.table)
colnames(final.table) <- c("OCR.error", "Total.error.detected", "Error.after.correction",
                           "before.recall", "before.precision", "after.recall", "after.precision")
write_csv(final.table, "../lib/final_csv.csv")
