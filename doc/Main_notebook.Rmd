...
In this stater code, we implement the first three rules in the first paper -- rule based techniques, as an example.

Load corpus into R
```{r}
load("../lib/words.RData")
load("../lib/tf.RData")
# Create deletion table from corpus
source("../lib/deletiontable.R")
```
...
false_word <- tesseract_vec[!tesseract_if_clean]
...
Load confusion matrices and existing RData
```{r}
sub_matrix <- read.csv("../data/sub_matrix.csv")
rownames(sub_matrix) <- letters
cm <- read.csv('../data/del_matrix.csv', row.names = 1, header = T) # deletion confusion matrix
...
```{r}
source('../lib/DELETION.R')
```

Get Substitution Candidate
```{r}
source('../lib/substitute.R')
```

Get Insertion Candidate
```{r, warning= FALSE}
source('../lib/insersion.R')
```

Combine all candidates and select the ones with highest scores
```{r}
source('../lib/Final candidate.R')
```

Replace error with candidate
```{r}
source("../lib/create new tess.R")
```

##Demonstration
```{r}
# All candidates of one word; using thls as an example
df$thls

# A quick look of all errors in this file with their best candidate
result[10:20, ]
```
...
new_intersect_vec <- vecsets::vintersect(tolower(ground_truth_vec), tolower(tesseract_vec1))
...
OCR_performance_table["word_wise_precision","Tesseract_with_postprocessing"] <- length(new_intersect_vec)/length(tesseract_vec1)
...
OCR_performance_table["character_wise_precision","Tesseract_with_postprocessing"] <- length(new_intersect_char)/length(tesseract_char1)

kable(OCR_performance_table, caption="Summary of OCR performance")
```

From the table, we see that for this file, there are decreases in word-level recall and precision; however, both performance measurement on letter-level increase. The possible explanation is that this algorithm proposes a single-letter-change as correction; even if it doesn't change the error into the true word, the letter being changed may match an error somewhere else in the file.

Due to the nature of parallel computation in R, we can't source our R script files like above while we are looping through all the text files. Instead, we had another file containing every piece of code used to generate results. See the chunk below.
```{r}
# these two files will generate 2 csv files containing evaluation paramters; those csv files are later saved in the /lib folder
source('../lib/mc run word level.R')
source("../lib/mc run char.R")
```

```{r}
final_csv <- read.csv("../lib/final_csv.csv")
final_csv_char <- read.csv("../lib/final_csv_char.csv")

total_performance_table <- data.frame("Tesseract" = rep(NA,4),
                                    "Tesseract_with_postprocessing" = rep(NA,4))
row.names(total_performance_table) <- c("word_wise_recall","word_wise_precision",
                                                 "character_wise_recall","character_wise_precision")
total_performance_table["word_wise_recall","Tesseract"] <- mean(final_csv$before.recall)
total_performance_table["word_wise_precision","Tesseract"] <- mean(final_csv$before.precision)
total_performance_table["word_wise_recall","Tesseract_with_postprocessing"] <- mean(final_csv$after.recall)
total_performance_table["word_wise_precision","Tesseract_with_postprocessing"] <- mean(final_csv$after.precision)

total_performance_table["character_wise_recall","Tesseract"] <- mean(final_csv_char$before.recall)
total_performance_table["character_wise_precision","Tesseract"] <- mean(final_csv_char$before.precision)
total_performance_table["character_wise_recall","Tesseract_with_postprocessing"] <- mean(final_csv_char$after.recall)
total_performance_table["character_wise_precision","Tesseract_with_postprocessing"] <- mean(final_csv_char$after.precision)

kable(total_performance_table, caption="Summary of Total OCR performance")
```

For the result from all 100 files, we see a similar pattern. The word-wise performance experienced a slight drop while letter-level performance improves.

# Potential improvement

  + The paper regarding correction is focusing on misspelling error instead of OCR error; therefore, some types of OCR errors don't have a proposed candidate. It is possible to use an entirely different correction algorithm.
  
  + The detection algorithm is rule-based and naive; it can miss some errors, and also classify correct words like "by" as errors due to rule violation. More advanced rules can be applied to improve detection performance.
  
  + This correction algorithm only propose 1-character change; so it cannot fix errors such as "paraiiel" while the correct word is "parallel". N-gram will be helpful in this case.
  
  + We only used the ground truth as our base dictionary; it contains 16902 tokens which is a relatively small amount. Increasing the dictionary size will boost the overall performance.
